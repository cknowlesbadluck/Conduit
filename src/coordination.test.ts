...
test("project creator can tombstone a foreign-owned resource; stranger cannot", async () => {
  await registerAgent({ id: "proj-creator", name: "Creator", actorSubject: "subject-proj-creator" });
  await registerAgent({ id: "res-owner", name: "Owner", actorSubject: "subject-res-owner" });
  await registerAgent({ id: "stranger", name: "Stranger", actorSubject: "subject-stranger" });
  const project = await createProject({ name: "Creator Project", createdBy: "proj-creator" });
  assert.ok(project);
  const resource = await registerResource({ projectId: project.id, name: "Foreign Resource", description: "owned by other agent", kind: "repository", endpoint: "https://github.com/example/foreign", createdBy: "res-owner" });
  assert.ok(resource);

  assert.equal(await archiveResource(resource.id, "stranger"), null);
  const archived = await archiveResource(resource.id, "proj-creator");
  assert.ok(archived?.archivedAt);
  assert.equal((await listResources()).some((item) => item.id === resource.id), false);
});

test("admin option can tombstone foreign-owned project and resource", async () => {
  await registerAgent({ id: "real-owner", name: "Owner", actorSubject: "subject-real-owner" });
  await registerAgent({ id: "admin-agent", name: "Admin", actorSubject: "subject-admin-agent" });
  const project = await createProject({ name: "Admin Target", createdBy: "real-owner" });
  assert.ok(project);
  const resource = await registerResource({ projectId: project.id, name: "Admin Resource", description: "stale", kind: "mcp", endpoint: "https://example.com/mcp", createdBy: "real-owner" });
  assert.ok(resource);

  assert.equal(await archiveResource(resource.id, "admin-agent"), null);
  assert.equal(await archiveProject(project.id, "admin-agent"), null);
  assert.ok((await archiveResource(resource.id, "admin-agent", { admin: true }))?.archivedAt);
  assert.ok((await archiveProject(project.id, "admin-agent", { admin: true }))?.archivedAt);
});
