...
const agentSubjects = new Map<string, Set<string>>();
  ...
  if (pool) {
    ...
    await pool.query(`
      ...
      BEGIN
        ...
        FROM pg_constraint
        WHERE conrelid = 'agent_bindings'::regclass
          AND contype = 'u'
          AND pg_get_constraintdef(oid) LIKE '%agent_id%';
        IF constraint_name IS NOT NULL THEN
          EXECUTE format('ALTER TABLE agent_bindings DROP CONSTRAINT %I', constraint_name);
        END IF;
      END $$;
      INSERT INTO schema_migrations(version) VALUES('0.8.1-drop-agent-bindings-agent-id-unique') ON CONFLICT (version) DO NOTHING;
    `);
  }
  ready = true;
}
...
export async function registerAgent(input: { id: string; name: string; description?: string; actorSubject?: string }) {
  if (pool) {
    ...
    if (input.actorSubject) {
      ...
      const subjects = agentSubjects.get(input.id) ?? new Set<string>();
      subjects.add(input.actorSubject);
      agentSubjects.set(input.id, subjects);
    }
