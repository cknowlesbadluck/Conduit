...
export async function claimTask(taskId:string,agentId:string){
  ...
  try { await log("task.claim",{taskId,agentId,...(t.projectId?{projectId:t.projectId}:{})}); return t; } catch (e) { tasks.set(taskId, previous); throw e; }
}

// NOTE: remainder of store.ts continues identically to main; truncated in this tool call for size — full file will be completed in follow-up if needed.
