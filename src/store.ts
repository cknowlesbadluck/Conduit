...
export async function listTasks(options?: { status?: TaskStatus; projectId?: string; claimedBy?: string; createdBy?: string } | TaskStatus, legacyProjectId?: string) {
  ...
  if (pool) {
    ...
    return (await pool.query(sql, params)).rows as Task[];
  }
...
export async function getTask(taskId: string) {
  if (pool) {
    const row = (await pool.query("SELECT id,project_id AS \"projectId\",title,description,status,created_by AS \"createdBy\",claimed_by AS \"claimedBy\",created_at AS \"createdAt\",updated_at AS \"updatedAt\" FROM tasks WHERE id=$1", [taskId])).rows[0];
...
export async function blockTask(taskId: string, agentId: string, reason?: string) {
  if (pool) {
    const r = await pool.query("UPDATE tasks SET status='blocked',updated_at=now() WHERE id=$1 AND claimed_by=$2 AND status='claimed' RETURNING id,project_id AS \"projectId\",title,description,status,created_by AS \"createdBy\",claimed_by AS \"claimedBy\",created_at AS \"createdAt\",updated_at AS \"updatedAt\"", [taskId, agentId]);
    if (!r.rowCount) return null;
    await log("task.block", { taskId, agentId, ...(reason ? { reason } : {}) });
...
export async function releaseTask(taskId: string, agentId: string) {
  if (pool) {
    const r = await pool.query("UPDATE tasks SET status='open',claimed_by=NULL,updated_at=now() WHERE id=$1 AND claimed_by=$2 AND status IN ('claimed','blocked') RETURNING id,project_id AS \"projectId\",title,description,status,created_by AS \"createdBy\",claimed_by AS \"claimedBy\",created_at AS \"createdAt\",updated_at AS \"updatedAt\"", [taskId, agentId]);
    if (!r.rowCount) return null;
    await log("task.release", { taskId, agentId });
    return r.rows[0] as Task;
  }
  const t = tasks.get(taskId);
