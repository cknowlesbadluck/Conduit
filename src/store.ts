...
export async function listSubjectsForAgent(agentId: string): Promise<string[]> {
  if (pool) {
    const rows = (await pool.query(
      "SELECT subject FROM agent_bindings WHERE agent_id=$1 ORDER BY subject ASC",
      [agentId],
    )).rows as { subject: string }[];
    return rows.map((r) => r.subject).filter((s) => typeof s === "string" && s.length > 0);
  }
  const subjects = boundAgentSubjects.get(agentId);
  return subjects ? [...subjects].filter((s) => s.length > 0).sort() : [];
}
