...
const agentBindingSubjects = new Map<string, Set<string>>();
    ...
    await pool.query(`
      ...
      BEGIN
        IF EXISTS (
          ...
          WHERE conname = 'agent_bindings_agent_id_key'
            AND conrelid = 'agent_bindings'::regclass
        ) THEN
          ALTER TABLE agent_bindings DROP CONSTRAINT agent_bindings_agent_id_key;
        END IF;
      END $$;
    `);
    await pool.query("INSERT INTO schema_migrations(version) VALUES('0.8.1') ON CONFLICT (version) DO NOTHING");
  }
  ready = true;
}
...
export async function registerAgent(input: { id: string; name: string; description?: string; actorSubject?: string }) {
  if (pool) {
    ...
    if (input.actorSubject) {
      ...
      const subjects = agentBindingSubjects.get(input.id) ?? new Set<string>();
      subjects.add(input.actorSubject);
      agentBindingSubjects.set(input.id, subjects);
    }
