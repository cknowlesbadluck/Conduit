...
export async function registerAgent(input: { id: string; name: string; description?: string; actorSubject?: string }) {
  if (pool) {
    const existing = (await pool.query("SELECT id,name,description,created_at AS \"createdAt\" FROM agents WHERE id=$1", [input.id])).rows[0] as Agent | undefined;
  ...
  const agent = pool
    ? (await pool.query("SELECT id,name,description,created_at AS \"createdAt\" FROM agents WHERE id=$1", [input.id])).rows[0] as Agent
    : agents.get(input.id)!;
...
async function projectExists(projectId: string) {
  if (pool) return Boolean((await pool.query("SELECT 1 FROM projects WHERE id=$1", [projectId])).rowCount);
...
export async function createProject(input: { name: string; description?: string; createdBy: string }) {
  ...
  await log("project.create", { projectId: p.id, agentId: p.createdBy });
  return p;
}

export async function listProjects() {
  if (pool) return (await pool.query("SELECT id,name,description,created_by AS \"createdBy\",created_at AS \"createdAt\",updated_at AS \"updatedAt\" FROM projects ORDER BY created_at DESC")).rows;
...
export async function registerResource(input: { projectId?: string; name: string; description: string; kind: string; endpoint?: string; createdBy: string }) {
  ...
  await log("resource.register", { resourceId:r.id, agentId:r.createdBy, ...(r.projectId ? {projectId:r.projectId} : {}) });
  return r;
}

export async function listResources(projectId?: string) {
  if (pool) return (await pool.query("SELECT id,project_id AS \"projectId\",name,description,kind,endpoint,created_by AS \"createdBy\",created_at AS \"createdAt\",updated_at AS \"updatedAt\" FROM resources" + (projectId ? " WHERE project_id=$1" : "") + " ORDER BY created_at DESC", projectId ? [projectId] : [])).rows;
  return [...resources.values()].filter(r => !projectId || r.projectId === projectId).sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
...
export async function createTask(input: { title: string; description?: string; createdBy: string; projectId?: string }) {
  ...
  await log("task.create", {taskId:t.id,agentId:t.createdBy,...(t.projectId?{projectId:t.projectId}:{})});
  return t;
}

export async function listTasks(status?: TaskStatus, projectId?: string) {
  if (pool) {
    const conditions:string[]=[]; const params:(string)[]=[];
    if(status){conditions.push(`status=$${params.length+1}`);params.push(status);} if(projectId){conditions.push(`project_id=$${params.length+1}`);params.push(projectId);}
    const sql="SELECT id,project_id AS \"projectId\",title,description,status,created_by AS \"createdBy\",claimed_by AS \"claimedBy\",created_at AS \"createdAt\",updated_at AS \"updatedAt\" FROM tasks"+(conditions.length?` WHERE ${conditions.join(" AND ")}`:"")+" ORDER BY created_at DESC";
    return (await pool.query(sql,params)).rows;
  }
  return [...tasks.values()].filter(t=>(!status||t.status===status)&&(!projectId||t.projectId===projectId)).sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}

export async function claimTask(taskId:string,agentId:string){
  if(!(await agentExists(agentId)))return null;
  if(pool){const r=await pool.query("UPDATE tasks SET status='claimed',claimed_by=$2,updated_at=now() WHERE id=$1 AND status='open' RETURNING id,project_id AS \"projectId\",title,description,status,created_by AS \"createdBy\",claimed_by AS \"claimedBy\",created_at AS \"createdAt\",updated_at AS \"updatedAt\"",[taskId,agentId]);if(!r.rowCount)return null;await log("task.claim",{taskId,agentId});return r.rows[0] as Task;}
  const t=tasks.get(taskId);if(!t||t.status!=="open")return null;t.status="claimed";t.claimedBy=agentId;t.updatedAt=now();await log("task.claim",{taskId,agentId});return t;
}

export async function completeTask(taskId:string,agentId:string){
  if(pool){const r=await pool.query("UPDATE tasks SET status='completed',updated_at=now() WHERE id=$1 AND claimed_by=$2 AND status='claimed' RETURNING id,project_id AS \"projectId\",title,description,status,created_by AS \"createdBy\",claimed_by AS \"claimedBy\",created_at AS \"createdAt\",updated_at AS \"updatedAt\"",[taskId,agentId]);if(!r.rowCount)return null;await log("task.complete",{taskId,agentId});return r.rows[0] as Task;}
  const t=tasks.get(taskId);if(!t||t.status!=="claimed"||t.claimedBy!==agentId)return null;t.status="completed";t.updatedAt=now();await log("task.complete",{taskId,agentId});return t;
}
...
export async function addContact(name:string,value:string,kind:string,projectId?:string,createdBy?:string){
  ...
  if(pool)await pool.query("INSERT INTO contacts(id,project_id,name,value,kind,created_by) VALUES($1,$2,$3,$4,$5,$6)",[c.id,c.projectId??null,name,value,kind,createdBy??null]);else contacts.unshift(c);
...
export async function registerTool(name:string,description:string,endpoint?:string,projectId?:string,createdBy?:string){
  ...
  if(pool)await pool.query("INSERT INTO tools(id,project_id,name,description,endpoint,created_by) VALUES($1,$2,$3,$4,$5,$6)",[t.id,t.projectId??null,name,description,endpoint??null,createdBy??null]);else tools.unshift(t);
...
export async function getCoordinationContext(projectId?:string){
  const project=projectId?(pool?(await pool.query("SELECT id,name,description,created_by AS \"createdBy\",created_at AS \"createdAt\",updated_at AS \"updatedAt\" FROM projects WHERE id=$1",[projectId])).rows[0]:projects.get(projectId)):undefined;
  if(projectId&&!project)return null;
  const [agentList,taskList,contactList,toolList,resourceList,activityList]=await Promise.all([listAgents(),listTasks(undefined,projectId),listContacts(projectId),listTools(projectId),listResources(projectId),listActivity(50,projectId)]);
  return {service:"Conduit",generatedAt:now(),...(projectId?{project}:{projects:await listProjects()}),agents:agentList,tasks:taskList,contacts:contactList,tools:toolList,resources:resourceList,activity:activityList};
}
