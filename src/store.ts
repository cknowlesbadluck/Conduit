...
export async function listContacts(projectId?: string) {
  if (pool) {
    const where = projectId ? " WHERE project_id=$1 AND archived_at IS NULL" : " WHERE archived_at IS NULL";
    return (
      await pool.query(
        "SELECT id,project_id AS \"projectId\",name,value,kind,created_by AS \"createdBy\",created_at AS \"createdAt\",archived_at AS \"archivedAt\" FROM contacts" + where + " ORDER BY created_at DESC, id DESC",
        projectId ? [projectId] : []
      )
    ).rows.map(normalizeContact);
  }
  return contacts
    .filter(c => !c.archivedAt && (!projectId || c.projectId === projectId))
...
export async function listTools(projectId?: string) {
  if (pool) {
    ...
    return (
      await pool.query(
        "SELECT id,project_id AS \"projectId\",name,description,endpoint,created_by AS \"createdBy\",created_at AS \"createdAt\",archived_at AS \"archivedAt\" FROM tools" + where + " ORDER BY created_at DESC, id DESC",
        projectId ? [projectId] : []
      )
    ).rows.map(normalizeTool);
  }
  return tools
    .filter(t => !t.archivedAt && (!projectId || t.projectId === projectId))
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}
