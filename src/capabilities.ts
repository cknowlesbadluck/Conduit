...
export function assertCapability(grants: CapabilityGrant[], request: CapabilityRequest): void {
  if (!grants.some((grant) => matchesCapability(grant, request))) {
    throw new Error(
      `capability_denied: no grant for agent=${request.agentId} provider=${request.provider} method=${request.method} path=${request.path}` +
        (request.projectId ? ` projectId=${request.projectId}` : "") +
        ". pathPattern supports exact, segment *, trailing prefix *, and /**. Regex (^ or .*) never matches.",
    );
  }
}
