...
test('loadAuthConfig rejects HTTP discovery URL in production', async () => {
  ...
  try {
    ...
    await assert.rejects(async () => loadAuthConfig(), (err: Error) => err.message.includes('must use HTTPS in production'));
  } finally {
...
test('loadAuthConfig rejects insecure discovered endpoints in production', async () => {
  ...
  try {
    ...
    globalThis.fetch = (async () => ({ ok: true, json: async () => ({ issuer: 'http://api.descope.com', jwks_uri: 'https://api.descope.com/jwks', authorization_endpoint: 'https://api.descope.com/auth', token_endpoint: 'https://api.descope.com/token' }) })) as unknown as typeof fetch;
    await assert.rejects(async () => loadAuthConfig(), (err: Error) => err.message.includes('must use HTTPS in production'));
    globalThis.fetch = (async () => ({ ok: true, json: async () => ({ issuer: 'https://api.descope.com', jwks_uri: 'http://api.descope.com/jwks', authorization_endpoint: 'https://api.descope.com/auth', token_endpoint: 'https://api.descope.com/token' }) })) as unknown as typeof fetch;
    await assert.rejects(async () => loadAuthConfig(), (err: Error) => err.message.includes('must use HTTPS in production'));
  } finally {
    process.env.NODE_ENV = origEnv;
    process.env.DESCOPE_MCP_SERVER_WELL_KNOWN_URL = origDiscovery;
...
test('loadAuthConfig validates DESCOPE_MCP_SERVER_ISSUER match and scheme', async () => {
  ...
  try {
    ...
    globalThis.fetch = (async () => ({ ok: true, json: async () => ({ issuer: 'https://api.descope.com', jwks_uri: 'https://api.descope.com/jwks', authorization_endpoint: 'https://api.descope.com/auth', token_endpoint: 'https://api.descope.com/token' }) })) as unknown as typeof fetch;
    await assert.rejects(async () => loadAuthConfig(), (err: Error) => err.message.includes('does not match the issuer returned by discovery'));
  } finally {
    process.env.NODE_ENV = origEnv;
    process.env.DESCOPE_MCP_SERVER_WELL_KNOWN_URL = origDiscovery;
