...
export function createConduitServer(authConfig?: ConduitAuthConfig) {
  ...
  server.registerTool("agent_identity", { description: "Return the authenticated actor, granted Conduit scopes, bound logical agent id, binding keys tried, and subjects holding the bound agent when one exists.", inputSchema: z.object({}), annotations: readOnly }, async (_input, extra) => {
    ...
    const bindingKeys = actorBindingLookupKeys(clientId, sub);
    ...
    return json({
      ...
      boundAgentId,
      bindingKeys,
      subjectsHoldingBoundAgent,
      bindingConflict: Boolean(boundAgentId && subjectsHoldingBoundAgent.length > 1),
  ...
  server.registerTool("agent_register", { description: "Register a logical agent identity and bind it to the authenticated actor. Subsequent writes use this bound identity.", inputSchema: z.object({ id: z.string().min(1).max(200), name: z.string().min(1).max(200), description: z.string().max(2000).optional() }), annotations: writeIdempotent }, async ({ id, name, description }, extra) => {
    ...
    if (actor && existing && existing !== id) {
      const subjects = await listSubjectsForAgent(existing);
      return rejected("agent_identity_already_bound", { boundAgentId: existing, requestedAgentId: id, subjectsHoldingBoundAgent: subjects, hint: "This actor is already bound to another logical agent. Use the existing boundAgentId or contact a grant admin." });
    }
    const result = await registerAgent({ id, name, description, actorSubject: actor });
    if (result) return json(result);
    const subjects = await listSubjectsForAgent(id);
    return rejected("agent_identity_conflict", { agentId: id, subjectsHoldingAgent: subjects, hint: "Logical agent id may already be bound to a different actor subject. Prefer a distinct agent id (e.g. grok-xai) or resolve the existing binding." });
  });
