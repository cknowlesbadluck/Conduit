...
export function createConduitServer(authConfig?: ConduitAuthConfig) {
  ...
  server.registerTool("agent_identity", { description: "Return the authenticated actor, granted Conduit scopes, bound logical agent id, and binding collision diagnostics when a bound agent exists.", inputSchema: z.object({}), annotations: readOnly }, async (_input, extra) => {
    ...
    const bindingConflict = Boolean(
      boundAgentId &&
      subjectsHoldingBoundAgent.some((subject) => subject.length > 0 && !bindingKeys.includes(subject))
    );
    return json({ clientId: info?.clientId ?? "unknown", scopes: info?.scopes ?? [], subject: sub, name: typeof info?.extra?.name === "string" ? info.extra.name : undefined, email: typeof info?.extra?.email === "string" ? info.extra.email : undefined, boundAgentId, bindingKeys, subjectsHoldingBoundAgent, bindingConflict, expiresAt: info?.expiresAt });
  });
  ...
  server.registerTool("agent_register", { description: "Register a logical agent identity and bind it to the authenticated actor. Subsequent writes use this bound identity.", inputSchema: z.object({ id: z.string().min(1).max(200), name: z.string().min(1).max(200), description: z.string().max(2000).optional() }), annotations: writeIdempotent }, async ({ id, name, description }, extra) => {
    ...
    const subjectsHoldingBoundAgent = await listSubjectsForAgent(id);
    return rejected("agent_identity_conflict", {
      agentId: id,
      subjectsHoldingBoundAgent,
      hint: "Another actor already holds this logical agent. Inspect agent_identity bindingKeys/subjectsHoldingBoundAgent, or register a distinct agent id.",
    });
  });
