...
type ErrorCode =
  | "agent_identity_not_bound"
  | "agent_identity_already_bound"
  | "agent_identity_conflict"
  | "agent_identity_not_bound_or_impersonation"
  | "project_not_found"
  | "project_not_found_or_agent_unregistered"
  | "project_creator_not_registered"
  | "task_unavailable_or_agent_unregistered"
  | "task_not_owned_or_not_claimed"
  | "handoff_rejected"
  | "task_not_found"
  | "task_not_claimable"
...
const rejected = (code: ErrorCode, message?: string, details?: Record<string, unknown>) => ({
  ...json({ error: { code, message: message ?? code, ...(details ? { details } : {}) } }),
...
export function createConduitServer(authConfig?: ConduitAuthConfig) {
  ...
  server.registerTool("conduit_context", { description: "Return Conduit purpose plus a live coordination snapshot (agents, projects, tasks, resources, tools, contacts, recent activity). Pass projectId to scope the snapshot to one project; omit for global view. Read-only.", inputSchema: z.object({ projectId: z.string().min(1).optional() }), annotations: { readOnlyHint: true } }, async ({ projectId }, extra) => {
  ...
  server.registerTool("agent_identity", { description: "Return the authenticated actor (OAuth sub/client), bound logical Conduit agent ID if any, and granted scopes. Call this first after connecting to confirm identity binding. Read-only.", inputSchema: z.object({}), annotations: { readOnlyHint: true } }, async (_input, extra) => {
    if (readScope) auth(extra, readScope);
    const info = extra.http?.authInfo;
  ...
  server.registerTool("agent_register", { description: "Register a logical Conduit agent identity and bind it to the authenticated OAuth actor. An actor cannot bind to an agent already bound to a different actor. Required before most write operations.", inputSchema: z.object({ id: z.string().min(1).max(200), name: z.string().min(1).max(200), description: z.string().max(2000).optional() }), annotations: { destructiveHint: false, readOnlyHint: false } }, async ({ id, name, description }, extra) => {
  ...
  server.registerTool("integration_call", { description: "Call a configured external provider API (github, render, supabase) using server-held credentials. Path must start with /. Read methods are GET/HEAD; write methods require write scope. Does not store secrets in records. Prefer list integrations_list first.", inputSchema: z.object({ provider: z.enum(integrationProviders), method: z.enum(integrationMethods), path: z.string().min(1).max(2000), body: z.unknown().optional(), projectId: z.string().min(1).max(200).optional() }), annotations: { destructiveHint: true, readOnlyHint: false } }, async ({ provider, method, path, body, projectId }, extra) => {
    ...
    try {
      ...
      return json(result);
    } catch (error) { const message = error instanceof Error ? error.message : "integration_call_failed"; console.warn(JSON.stringify({ type: "integration.call.failed", provider, method, path, projectId: projectId ?? null, actor: actorSubject(extra) ?? null, error: message })); return rejected("integration_failed", message); }
  });

  server.registerTool("mcp_bridge_call", { description: "Forward a JSON-RPC request to a remote HTTPS MCP endpoint. Blocks localhost and private networks. Size and timeout limited. Use for trusted remote MCP servers only; does not inject Conduit credentials into the target.", inputSchema: z.object({ endpoint: z.string().url(), method: z.string().min(1).max(200), id: z.union([z.string(), z.number(), z.null()]).optional(), params: z.unknown().optional(), projectId: z.string().min(1).max(200).optional() }), annotations: { destructiveHint: true, readOnlyHint: false } }, async ({ endpoint, method, id, params, projectId }, extra) => {
    if (readScope) auth(extra, readScope);
    try {
      ...
      return json(result);
    } catch (error) { const message = error instanceof Error ? error.message : "mcp_bridge_call_failed"; console.warn(JSON.stringify({ type: "mcp.bridge.call.failed", endpoint, method, projectId: projectId ?? null, actor: actorSubject(extra) ?? null, error: message })); return rejected("mcp_bridge_failed", message); }
  });
  ...
  server.registerTool("task_claim", { description: "Atomically claim an open task for the bound agent. Fails if the task is missing, already claimed, completed, or the agent is not bound. Prefer this over concurrent claims.", inputSchema: z.object({ taskId: z.string().min(1), agentId: z.string().min(1).optional() }), annotations: { destructiveHint: false, readOnlyHint: false } }, async ({ taskId, agentId }, extra) => {
  ...
  server.registerTool("task_complete", { description: "Mark a task completed. Only the agent that currently claims the task may complete it. Use task_handoff first if another agent should finish the work.", inputSchema: z.object({ taskId: z.string().min(1), agentId: z.string().min(1).optional() }), annotations: { destructiveHint: true, readOnlyHint: false } }, async ({ taskId, agentId }, extra) => {
    if (writeScope) auth(extra, writeScope); const actor = await requireBoundAgent(extra, agentId); if (!actor) return rejected("agent_identity_not_bound_or_impersonation");
    const result = await completeTask(taskId, actor); return result ? json(result) : rejected("task_not_owned_or_not_claimed");
  });
  server.registerTool("task_handoff", { description: "Transfer ownership of a claimed task from the bound agent to another registered agent. Requires the caller to currently own the task. Optional note is recorded in activity. Destructive.", inputSchema: z.object({ taskId: z.string().min(1), fromAgent: z.string().min(1).optional(), toAgent: z.string().min(1), note: z.string().max(5000).optional() }), annotations: { destructiveHint: true, readOnlyHint: false } }, async (input, extra) => {
