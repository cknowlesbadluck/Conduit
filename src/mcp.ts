...
import { registerGrantTools, isGrantAdmin } from "./grant-tools.js";
...
export function createConduitServer(authConfig?: ConduitAuthConfig) {
  ...
  server.registerTool("project_archive", { description: "Archive a project. Allowed for the project creator or CONDUIT_GRANT_ADMIN_SUBJECTS. Tombstone only; records are not hard-deleted.", inputSchema: z.object({ projectId: z.string().min(1), agentId: z.string().min(1).max(200).optional() }), annotations: writeIdempotent }, async ({ projectId, agentId }, extra) => {
