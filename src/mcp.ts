...
const auth = (extra: { authInfo?: AuthInfo }, scope: string) => requireScope(extra.authInfo, scope);
...
export function createConduitServer(authConfig?: ConduitAuthConfig) {
  ...
  server.registerTool(
    ...
    async (input, extra) => {
      if (writeScope) auth(extra, writeScope);
      return json(await createTask(input));
    },
  );
  ...
  server.registerTool(
    "task_claim",
    { description: "Atomically claim an open task", inputSchema: z.object({ taskId: z.string().min(1), agentId: z.string().min(1) }), annotations: { destructiveHint: false, readOnlyHint: false } },
    async ({ taskId, agentId }, extra) => {
      if (writeScope) auth(extra, writeScope);
      const result = await claimTask(taskId, agentId);
      return result ? json(result) : { ...json({ error: "task_unavailable" }), isError: true };
    },
  );

  server.registerTool(
    "task_complete",
    { description: "Complete a task claimed by the calling agent", inputSchema: z.object({ taskId: z.string().min(1), agentId: z.string().min(1) }), annotations: { destructiveHint: true, readOnlyHint: false } },
    async ({ taskId, agentId }, extra) => {
      if (writeScope) auth(extra, writeScope);
  ...
  server.registerTool(
    "task_handoff",
    { description: "Hand a task from its current agent to another agent", inputSchema: z.object({ taskId: z.string().min(1), fromAgent: z.string().min(1), toAgent: z.string().min(1), note: z.string().optional() }), annotations: { destructiveHint: true, readOnlyHint: false } },
    async (input, extra) => {
      if (writeScope) auth(extra, writeScope);
  ...
  server.registerTool(
    "contact_add",
    { description: "Store a shared contact or resource reference", inputSchema: z.object({ name: z.string().min(1), value: z.string().min(1), kind: z.string().min(1) }), annotations: { destructiveHint: false, readOnlyHint: false } },
  ...
  server.registerTool(
    "tool_register",
    { description: "Register a shared tool or MCP endpoint", inputSchema: z.object({ name: z.string().min(1), description: z.string().min(1), endpoint: z.string().url().optional() }), annotations: { destructiveHint: false, readOnlyHint: false } },
