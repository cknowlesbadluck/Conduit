...
export function createConduitServer(authConfig?: ConduitAuthConfig) {
  ...
  server.registerTool("project_archive", { description: "Archive a project. Allowed for the project creator or a CONDUIT_GRANT_ADMIN_SUBJECTS actor. Tombstone only; records are not hard-deleted.", inputSchema: z.object({ projectId: z.string().min(1), agentId: z.string().min(1).max(200).optional() }), annotations: writeIdempotent }, async ({ projectId, agentId }, extra) => {
    ...
    const subjects = [actorSubject(extra), oauthSubject(extra)].filter((value): value is string => Boolean(value));
    const result = await archiveProject(projectId, actor, { actorSubjects: subjects }); return result ? json(result) : rejected("project_not_found_or_not_owned", { projectId });
  });
  ...
  server.registerTool("resource_archive", { description: "Archive a resource. Allowed for the resource creator, the parent project creator, or a CONDUIT_GRANT_ADMIN_SUBJECTS actor. Tombstone only; records are not hard-deleted.", inputSchema: z.object({ resourceId: z.string().min(1), agentId: z.string().min(1).max(200).optional() }), annotations: writeIdempotent }, async ({ resourceId, agentId }, extra) => {
    if (writeScope) auth(extra, writeScope); const actor = await resolveBoundAgent(extra, agentId); if (!actor) return rejected("agent_identity_not_bound");
    const subjects = [actorSubject(extra), oauthSubject(extra)].filter((value): value is string => Boolean(value));
    const result = await archiveResource(resourceId, actor, { actorSubjects: subjects }); return result ? json(result) : rejected("resource_not_found_or_not_owned", { resourceId });
  });
