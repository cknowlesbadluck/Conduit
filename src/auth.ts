...
export async function loadAuthConfig(): Promise<ConduitAuthConfig | null> {
  ...
  const enabled = Boolean(discoveryUrl);
  ...
  if (!metadata.issuer || !metadata.jwks_uri || !metadata.authorization_endpoint || !metadata.token_endpoint) {
  ...
  if (configuredIssuer && configuredIssuer !== metadata.issuer) {
